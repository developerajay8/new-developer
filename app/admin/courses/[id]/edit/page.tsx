...
export default function EditCoursePage() {
      ...
      .then((data) => {
        ...
        setSections(
          Array.isArray(data.sections) && data.sections.length > 0
            ? data.sections.map((s: SectionForm) => ({
                ...s,
                lessons: s.lessons.map((l: LessonForm) => ({ ...l, videoUrl: l.videoUrl ?? '' })),
              }))
            : []
        );
      })
  ...
  if (error && !title) {
    return (
      <div>
        <Link href="/admin/courses" className="text-[#C9A961] font-medium hover:underline">Back to courses</Link>
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-serif font-bold text-[#C9A961] mb-6">Edit course</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border-2 border-[#C9A961]/30 p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:border-[#C9A961] outline-none text-gray-900 placeholder:text-gray-500 bg-white min-h-[80px]"
            />
          </div>
        ...
        <div className="bg-white rounded-xl border-2 border-[#C9A961]/30 p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#C9A961]/20 pb-2">
            ...
            <button type="button" onClick={addSection} className="flex items-center gap-1 text-[#C9A961] font-medium text-sm hover:underline">
            ...
            <div key={section.id} className="border border-gray-200 rounded-lg p-4 space-y-3">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border-2 border-gray-300 rounded-lg focus:border-[#C9A961] outline-none text-gray-900 placeholder:text-gray-500 bg-white text-sm"
                  placeholder="Section title"
                />
                <button type="button" onClick={() => removeSection(section.id)} className="p-2 rounded hover:bg-red-50 text-red-600">
                ...
                {section.lessons.map((lesson) => {
                  ...
                  return (
                    <div key={lesson.id} className="flex flex-wrap items-center gap-2 text-sm">
                      ...
                      />
                      <button type="button" onClick={() => removeLesson(section.id, lesson.id)} className="p-1.5 rounded hover:bg-red-50 text-red-600">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  );
                })}
                <button type="button" onClick={() => addLesson(section.id)} className="flex items-center gap-1 text-[#C9A961] text-xs font-medium hover:underline">
        ...
        <div className="flex gap-3">
          ...
          <Link href="/admin/courses" className="px-6 py-2.5 border-2 border-[#C9A961]/50 rounded-lg font-medium text-black/80 hover:border-[#C9A961]">
            Cancel
          </Link>
        </div>
      </form>
